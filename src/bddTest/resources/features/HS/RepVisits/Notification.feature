...
#Verify the City and State displayed in the HE Activity Notification Tab
  ...
  @MATCH-1639
  Scenario Outline: As a high school RepVisits user, I want Naviance to send visit information to Naviance when I create or modify a visit
  So that Naviance accurately reflects my visits.
  #pre-condition
    Given HS I am logged in to Intersect HS through Naviance with user type "navianceAdmin"
    And HS I go to the Naviance Settings to select the option "Automatically publish confirmed visits." and fill the following details "<NumVisits>","<Location>","<studentsCount>","<DeadlineValue>","<Notes>","<deadlineOption>"
  #create new visit for Reschedule
    Then HS I set a date using "<StartDateforNewVisit>" and "<EndDateforNewVisit>"
    Then HS I add the new time slot with "<DayforNewVisit>","<StartTimefornewVisit>","<EndTimefornewVisit>" and "<NumVisits>" with "1"
    Then HS I set the value for Reschedule the visit
  #create Visit
    Then HS I set a date using "<StartDate>" and "<EndDate>"
    And HS I verify the update button appears and I click update button
    Then HS I add the new time slot with "<Day>","<StartTime>","<EndTime>" and "<NumVisits>" with "1"
    Then HS I set the RepVisits Visits Confirmations option to "<Option>"
    And HS I set the Visit Availability of RepVisits Availability Settings to "All RepVisits Users"
    Then HS I set the Prevent colleges scheduling new visits option of RepVisits Visit Scheduling to "1"
    Then HS I set the Prevent colleges cancelling or rescheduling option of RepVisits Visit Scheduling to "1"
    ...
    And HS I successfully sign out
  #schedule Visit
    Given HE I want to login to the HE app using "purpleheautomation+Alpena@gmail.com" as username and "Password!1" as password
    And HE I search for "<School>" in RepVisits page
    Then HE I select Visits to schedule the appointment for "<School>" using "<Date>" and "<heStartTime>"
    And HE I verify the schedule pop_up for "<School>" using "<heTime>" and "<hsEndTime>"
  #verify Newly created visit is present In Naviance
    Given HS I go to the Naviance Page with user type "navianceAdmin"
    Then HS I select "view" In Naviance college visit Page using "<Date>","<Time>"
    And HS I verify the naviance college visit page using the following details "<University>","<Date>","<PrivateNotes>","<Representative>","<Location>","<Notes>","<studentsCount>","<Deadline>","<Attendee>"
  ...
  #reschedule visit
    ...
    Then HS I reschedule a visit for the following details "<heTimefornewVisit>","<reason>","<StartDateforNewVisit>"
    Then HS I remove the Time Slot created with "<StartDate>","<StartTime>" in Regular Weekly Hours Tab
    And HS I successfully sign out
  #verify updated visit Details In Naviance
    Given HS I go to the Naviance Page with user type "navianceAdmin"
    Then HS I select "view" In Naviance college visit Page After Reschedule the visit using "<StartDateforNewVisit>","<NewVisitTime>"
    And HS I verify the naviance college visit page using the following details after Reschedule the Visits "<University>","<StartDateforNewVisit>","<PrivateNotes>","<Representative>","<Location>","<Notes>","<studentsCount>","<Deadline>","<Attendee>"
    And HS I successfully sign out from the Naviance
  #Remove the appointment from Calendar
    ...
    And HS I successfully sign out
    Examples:
      |Day|DayforNewVisit|StartTime|EndTime |NumVisits|Option                            |hsEndTime|School                   |University               |heStartTime   |heTime   |Date |StartDate|StartDateforNewVisit|EndDate |EndDateforNewVisit|StartTimefornewVisit |EndTimefornewVisit|User     |reason   |heTimefornewVisit|heCalendarTime|Representative   |Location|Time     |NewVisitTime|Notes              |Attendee|Deadline                   |studentsCount|deadlineOption|PrivateNotes|DeadlineValue|
      |14 |21            |10:58am  |12:59pm |3        |Yes, accept all incoming requests.|12:59pm  |Int Qa High School 4     |Alpena Community College |10:58am       |10:58am  |14   |14       |21                  |35      |42                |10:34 am             |12:33 pm          |Purple HE|by QA    |10:34am          |10:58AM       |Purple HE        |Cbba    |10:58 AM |10:34 AM    |sss-testing from HS|N/A     |no later than 2 hours prior|30           |hours         |N/A         |2            |